...
class LNSBMutation(Mutation):
    ...
    def mutate(self, chromosome: Chromosome) -> Chromosome:
        log.info(f"Before Mutate: {chromosome.routes}")
        distance_matrix = Dataset.distance
        time_matrix = Dataset.distance
        ...
        queue = [LNSBMutation.Node(permutation, LNSBMutation.boundary(
        ...
        while queue:
            ...
            if current_node.lower_bound < best_value:
                if len(current_node.solution) == len(permutation):
                    current_value = LNSBMutation.objective_function(
                    ...
                    # 분기 및 자식 노드 생성
                    children = LNSBMutation.branch(
                        current_node, distance_matrix, time_matrix)
                    queue.extend(children)

        # 찾은 최적해 반환
        result = Chromosome(best_solution)
        log.info(f"After Mutate: {result.routes}")
        return result

    @staticmethod
    def boundary(solution, distance_matrix, time_matrix):
        # 경계 함수 계산 함수
        total_distance = LNSBMutation.calculate_total_distance(solution, distance_matrix)
        total_time = LNSBMutation.calculate_total_time(solution, time_matrix)
    ...
    def calculate_total_distance(solution, distance_matrix):
        ...
        for i in range(len(solution) - 1):
            ...
            if from_customer is not None and to_customer is not None:
                total_distance += Dataset.distance[from_customer][to_customer]
            else:
    ...
    def calculate_total_time(solution, time_matrix):
        ...
        for i in range(len(solution) - 1):
            ...
            if from_customer is not None and to_customer is not None:
                total_time += Dataset.distance[from_customer][to_customer]
            else:
                # 예외 처리: from_customer 또는 to_customer가 None인 경우 0 반환 또는 다른 처리 수행
                return 0
    ...
    def branch(node, distance_matrix, time_matrix):
        ...
        for i in range(len(node.solution)):
            for j in range(i + 1, len(node.solution)):
                ...
                child_lower_bound = LNSBMutation.boundary(
                    child_solution, distance_matrix, time_matrix)
                child_node = LNSBMutation.Node(child_solution, child_lower_bound)
